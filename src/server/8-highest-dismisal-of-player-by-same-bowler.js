...
function highestDismissedPlayerByParticularBowler(deliveries) {
    try {
        ...
        const listMostDismissedPlayer = sortListDismissedPlayerBowlerCount.slice(0,11);

        let mostDismissedPlayer = {};
        listMostDismissedPlayer.forEach((current)=>{
            mostDismissedPlayer[current[0]] = Object.entries(current[1])[0];
        })
